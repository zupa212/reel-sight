-- Phase 1: Database Schema Updates for ModelFlow (Simplified)
...
-- Update reel_metrics_daily to add missing metrics
ALTER TABLE public.reel_metrics_daily
ADD COLUMN IF NOT EXISTS day date,
ADD COLUMN IF NOT EXISTS watch_time_seconds integer;

-- Set default value for day column based on existing data if any
...
CREATE INDEX IF NOT EXISTS reel_metrics_daily_reel_day_idx 
ON public.reel_metrics_daily (reel_id, day DESC);
