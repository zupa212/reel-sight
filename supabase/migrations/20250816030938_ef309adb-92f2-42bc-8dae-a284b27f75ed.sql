-- Fix security warnings by moving materialized views to a private schema
-- This prevents direct API access while still allowing RPC functions to use them

-- Create a private schema for materialized views
CREATE SCHEMA IF NOT EXISTS private;

-- Drop existing materialized views from public schema
DROP MATERIALIZED VIEW IF EXISTS public.mv_model_stats_7d CASCADE;
DROP MATERIALIZED VIEW IF EXISTS public.mv_model_stats_30d CASCADE;
DROP MATERIALIZED VIEW IF EXISTS public.mv_top_reels_30d CASCADE;
DROP MATERIALIZED VIEW IF EXISTS public.mv_reels_cadence_30d CASCADE;

-- Recreate materialized views in private schema
CREATE MATERIALIZED VIEW private.mv_model_stats_7d AS
SELECT 
    r.model_id,
...
CREATE MATERIALIZED VIEW private.mv_model_stats_30d AS
SELECT 
    r.model_id,
...
FROM public.reels r
LEFT JOIN public.reel_metrics_daily rmd ON r.id = rmd.reel_id
LEFT JOIN public.models m ON r.model_id = m.id
WHERE r.posted_at >= NOW() - INTERVAL '30 days'
GROUP BY r.model_id, m.username;

CREATE MATERIALIZED VIEW private.mv_top_reels_30d AS
SELECT 
    r.id as reel_id,
    r.model_id,
    r.caption,
...
CREATE MATERIALIZED VIEW private.mv_reels_cadence_30d AS
SELECT 
    r.model_id,
...
CREATE INDEX idx_private_mv_model_stats_7d_model_id ON private.mv_model_stats_7d(model_id);
CREATE INDEX idx_private_mv_model_stats_30d_model_id ON private.mv_model_stats_30d(model_id);
CREATE INDEX idx_private_mv_top_reels_30d_views ON private.mv_top_reels_30d(total_views DESC);
CREATE INDEX idx_private_mv_reels_cadence_30d_model_date ON private.mv_reels_cadence_30d(model_id, post_date);

-- Update the api_dashboard_bundle function to use private schema
CREATE OR REPLACE FUNCTION public.api_dashboard_bundle(model_ids uuid[] DEFAULT NULL)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
